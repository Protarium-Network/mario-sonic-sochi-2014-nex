...
func (commonProtocol *CommonProtocol) autoMatchmakeWithSearchCriteriaPostpone(err error, packet nex.PacketInterface, callID uint32, lstSearchCriteria types.List[match_making_types.MatchmakeSessionSearchCriteria], anyGathering match_making_types.GatheringHolder, strMessage types.String) (*nex.RMCMessage, *nex.Error) {
	...
	// Nintendo's Wii Sports Club matchmaking replies contain an empty
	// MatchmakeParam map. The common Pretendo database layer adds the generic
	// @SR and @GIR parameters while creating a session; those values are useful
	// internally but are not part of this title's retail wire response. Sending
	// them to the game leaves PIA connected while the application session never
	// advances past its Type0 keep-alives. Strip only the response copy so the
	// database and other matchmaking bookkeeping remain untouched.
	responseSession.MatchmakeParam = match_making_types.NewMatchmakeParam()
	...
	common_globals.Logger.Info(fmt.Sprintf("[WSC-WIRE] AutoMatchmake response GID=%d PID=%d GameMode=%d AppBuffer=%X MatchmakeParam=%X Body=%X",
		uint32(responseSession.Gathering.ID), uint64(connection.PID()), uint32(responseSession.GameMode),
		[]byte(responseSession.ApplicationBuffer), []byte{}, rmcResponseBody))
